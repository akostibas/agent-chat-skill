...
2. Coordination requires that a participant can send a message to a
   specific recipient and trust that it reaches someone able to respond in
   a timely manner — a message that silently goes nowhere is worse than one
   that fails loudly.
3. Attention is the machine's scarcest resource — human and agent alike —
   and modest effort spent anywhere in the system (say, addressing messages
   deliberately) is a good trade for preserving it.
4. Humans are working parts of the machine, not spectators: when work hits
   ...
   autonomously.
5. The system stays cheap and simple to use so that it stays cheap and
...
6. **A human-facing interface.** Humans participate through their own
...
7. **A permanent record.** History exists so a participant can catch up,
   not as an archive — idle conversations are allowed to disappear.
8. **Adversarial identity.** Peers are trusted collaborators inside one
   security perimeter, and identity needs only to be good enough that
   messages land on the right desk — not proof against deliberate
   impersonation.
9. **Work orchestration.** How work gets broken down, assigned, and
   tracked is a convention layered on top — the channel itself stays a
   messaging substrate and never becomes the scheduler.
