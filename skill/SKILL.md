...
- **A mis-addressed send is refused, not quietly dropped.** If the body has an unquoted `@`-token but *no* present member matches it (a typo, or a peer who hasn't joined), the send is refused (exit 2) — fix the name, quote it (below), use `@all`, or drop the `@` to make it an FYI. The tell is the `@` itself: an `@`-token means you *meant* to address someone, so a non-match is treated as a mistake; a body with **no** `@`-token at all is a deliberate FYI, not a mistake.
- **To write an `@`-token without addressing anyone, wrap it in backticks.** A token inside a markdown code span is content, not an address: ``​`@dependabot rebase`​`` and ``​`@vercel/otel`​`` send fine even though no such peer exists. Quoted tokens notify no one and don't count toward the addressed set, so `` @alice see `@vercel/otel` `` still wakes exactly `alice`. Fenced blocks (```` ``` ````) quote the same way. This is per-token, so a quoted token elsewhere never rescues a genuinely typo'd name in the same message — that still fails loudly. An unmatched backtick is literal text and quotes nothing, so a stray backtick can't silently swallow the addresses after it — but **keep your backticks balanced**: a stray one pairs with the *opening* backtick of a later quote, leaving that token unquoted and getting the send refused. If a send is refused for a token you thought you quoted, look for an odd backtick earlier in the body. Over-quoting always fails loudly; it never delivers to the wrong audience. (Issue #57; see `docs/adr/0013-code-span-mention-escape.md`.)
...
See `docs/adr/0012-fyi-pull-only-tier.md` for the FYI tier, `docs/adr/0013-code-span-mention-escape.md` for the quoting escape, and `docs/adr/0010-require-explicit-audience.md` for the addressing rules they amend (together they supersede `docs/adr/0001-default-broadcast-with-mention-narrowing.md`, which made broadcast the default).
