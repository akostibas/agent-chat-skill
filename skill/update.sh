#!/usr/bin/env bash
#
# update.sh — upgrade this skill to the latest GitHub release. Portable
# self-update pattern: to reuse in another skill, copy this file and change the
# REPO / SKILL_NAME vars below. The repo must ship a Makefile whose `install`
# target accepts a SKILL_DIR override and writes a VERSION file (see this
# project's Makefile).
#
# What it does:
...
# --- Portable configuration: edit these two to reuse in another skill ---------
...
# Current installed version from the VERSION file `make install` writes
# (best-effort; empty if this is an older install without one).
current=""
[[ -f "$SKILL_DIR/VERSION" ]] && current="$(cat "$SKILL_DIR/VERSION" 2>/dev/null || true)"
...
if ! $assume_yes; then
  if [[ -t 0 ]]; then
    ...
    echo
    echo "Re-run with --yes to upgrade (reinstalls the skill to the location above)."
    exit 0
  fi
fi
